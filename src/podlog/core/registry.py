...
def _build_file_handler(spec: HandlerSpec, paths: PathsConfig) -> logging.Handler:
    ...
    if isinstance(retention_cfg, dict):
        max_files_val = retention_cfg.get("max_files")
        max_days_val = retention_cfg.get("max_days")
        retention = RetentionPolicy(
            max_files=int(max_files_val) if max_files_val is not None else None,
            max_days=int(max_days_val) if max_days_val is not None else None,
